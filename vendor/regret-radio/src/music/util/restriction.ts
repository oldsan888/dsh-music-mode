/**
 * 不可播原因归类（§3.1，netease+qq 共用受限对象工厂）。纯函数，可单测。
 * 这是双源回退/试听兜底的契约来源（action: login/upgrade/purchase/switch_source），前端据此提示。
 * 行为保真：分支判定（fee/code/freeTrial、QQ 104003/会员关键词）不可漏，否则换源/试听提示错乱。
 */
...
/** QQ：按 session/playbackKey/code/会员关键词归类不可播原因。 */
export function classifyQQPlaybackRestriction(
  info: Record<string, unknown> | null | undefined,
  session: boolean | { hasSession?: boolean; hasPlaybackKey?: boolean },
): PlaybackRestriction {
